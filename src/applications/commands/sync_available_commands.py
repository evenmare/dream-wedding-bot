"""Module contains sync of available command implementation."""
...
class SyncAvailableCommandsCommand(BaseCommand):
    """Class implements logic for sync available commands command."""

    command_name = 'sync-available-commands'

    @inject
    async def __call__(
        self,
        guest_repository: GuestRepository = Provide[Container.guest_repository],
        update_available_commands_service: UpdateAvailableCommandsService = Provide[
            Container.update_available_commands_service
        ],
    ) -> None:
        """Runs a command.
        ...
        await update_available_commands_service(guests_ids=guests_ids)